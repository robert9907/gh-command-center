...
// Standalone script — overrides GH_1000.buildSystemPrompt when loaded via <script src>
...
window.GH_1000.buildSystemPrompt = function(pageType) {
  ...
  var checklist = [
    ...
    "",
    "AEO (6 checks):",
    "  □ Instant answer block (.gh-answer) — " + T.minimums.instant_answers + "+ blocks",
    "  □ FAQ schema (FAQPage in @graph) — " + T.minimums.faqs + "+ questions",
    "  □ HowTo schema with " + T.minimums.process_steps + "+ steps",
    "  □ Comparison table (.gh-comparison) present",
    "  □ Speakable schema markup on .gh-answer and .gh-faq-a",
    "  □ " + T.minimums.instant_answers + "+ answer boxes total",
    "",
    "SEO (8 checks):",
    "  □ Single H1 with primary keyword",
    "  □ H2 structure (5+ headings)",
    "  □ Internal links (" + T.minimums.internal_links + "+) to generationhealth.me pages",
    "  □ Word count " + T.minimums.word_count + "+",
    ...
    "  □ .gov citations (" + T.minimums.gov_citations + "+): " + govSources.join(", "),
    ...
    "  □ Case studies with $ amounts (" + T.minimums.case_studies + "+)",
    "  □ Process steps section (" + T.minimums.process_steps + "+ steps)",
    "  □ Expert tips section (first person, from Rob)",
    "  □ FAQs (" + T.minimums.faqs + "+ questions)",
    "  □ Visual elements (cost grid with " + T.minimums.cost_grid_cards + " cards)",
    "  □ Pain points section (" + T.minimums.pain_points + "+ consequence items)",
    "  □ Trust vs Red Flags (" + T.minimums.trust_items + "+ each)",
    "",
    "VQA (12 checks):",
    "  □ Hero Call button (.gh-hero-btn--call) with tel:" + B.phone_plain,
    "  □ Hero Compare button (.gh-hero-btn--compare) with correct CTA link",
    "  □ CTA modals (" + T.minimums.cta_modals + "+) with .gh-cta-modal class",
    ...
    "  □ Multiple CTAs (" + T.minimums.cta_modals + "+)",
    ...
    "  □ Word count " + T.minimums.word_count + "+",
    "  □ More FAQs than competitors (" + T.minimums.faqs + "+)",
  ...
  return [
    ...
    JSON.stringify({
      ...
      instant_answers: [{ label: "Quick Answer", text: "2-4 sentences with 2026 figure", source: ".gov citation" }],
      ...
      comparison_table: { title: "string", subtitle: "string", column_headers: ["string"], rows: [["cell values"]], winner: "string", footnote: "string" },
      case_studies: [{ tag: "category", tag_color: "red|green|blue|purple|orange", title: "string", situation: "string", outcome: "string", lesson: "string", dollar_amount: "$X,XXX" }],
      process_steps: [{ number: 1, title: "string", description: "string", time: "string" }],
      ...
      howto_steps: { name: "How to [action]", description: "string", steps: [{ name: "step title", text: "step description" }] },
      savings_programs: [{ name: "string", eligibility: "string", benefit: "string" }],
      faq: [{ question: "string", answer: "2-3 sentences", schema_answer: "concise version for JSON-LD" }],
